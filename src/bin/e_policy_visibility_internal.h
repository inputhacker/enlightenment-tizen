...
#define VS_INF(ec, f, x...) \
      INF("VISIBILITY | "f" | '%s'(%p) RscID %d", ##x, ec ? NAME(ec) : "", ec, ec ? e_pixmap_res_id_get(ec->pixmap) : 0)
#define VS_ERR(ec, f, x...) \
      ERR("VISIBILITY | "f" | '%s'(%p) RscID %d", ##x, ec ? NAME(ec) : "", ec, ec ? e_pixmap_res_id_get(ec->pixmap) : 0)
